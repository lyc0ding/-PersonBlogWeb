<template>
  <div id="space-container">
    <ScrollPositionKeeper
      ref="scrollPositionKeeper"
      :storage-key="SPACE_SCROLL_POSITION_KEY"
    />
    <div id="container">
      <ImagePreviewer
                ...
                <div>
                  ...
                  <h2 class="moment-title">
                    <RouterLink
                      ...
                      @click.stop="handleMomentLinkClick(item)"
                    >
                      {{ item.title || '无标题动态' }}
                    </RouterLink>
                  </h2>
...
import ScrollPositionKeeper from '@/components/layout/ScrollPositionKeeper.vue'
...
const SPACE_SCROLL_POSITION_KEY = 'personblog_space_scroll_top'
const loading = ref(false)
const error = ref('')
const moments = ref([])
const total = ref(0)
const scrollPositionKeeper = ref(null)
...
async function restoreMomentPosition() {
  ...
  if (Number(target.pageNum) !== query.pageNum || Number(target.pageSize) !== query.pageSize) {
    sessionStorage.removeItem(SPACE_SCROLL_TARGET_KEY)
    scrollPositionKeeper.value?.clear()
    return
  }

  sessionStorage.removeItem(SPACE_SCROLL_TARGET_KEY)
  await scrollPositionKeeper.value?.restore({ behavior: 'smooth', duration: 260 })
}
...
.space-layout {
  ...
  gap: 34px;
  align-items: start;
  animation: list-return-in 180ms ease-out both;
}
